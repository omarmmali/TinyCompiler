...
private:
    ...
    std::map <std::pair<int,T>, int> transitions; // The set of all transitions (Edges), which is a tuple of (source, edge value, destination) 
public:
    DFA();
    void add_state(int, bool);
    void add_transition(int, T, int);
    bool is_accepted();
    bool will_accept(T);
    void reset();
    int insert(T);
